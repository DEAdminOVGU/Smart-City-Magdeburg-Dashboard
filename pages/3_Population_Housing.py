...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 3: AGE STRUCTURE (POPULATION PYRAMID)
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(section_header("Age Structure", color=POP_BLUE), unsafe_allow_html=True)

try:
    df_ages = load_kiss("bevoelkerung/bevoelkerung-mit-hauptwohnsitz-nach-alter-und-geschlecht.json")
    age_yr = int(df_ages["Jahr"].max())
    df_ages_y = df_ages[df_ages["Jahr"] == age_yr].copy()

    male_col   = next((c for c in df_ages_y.columns if "männlich" in c.lower() or "maennlich" in c.lower()), None)
    female_col = next((c for c in df_ages_y.columns if "weiblich" in c.lower()), None)
    age_col_n  = "Alter (Jahre)" if "Alter (Jahre)" in df_ages_y.columns else next(
                     (c for c in df_ages_y.columns if "alter" in c.lower()), None)

    if male_col and female_col and age_col_n:
        df_ages_y[age_col_n] = pd.to_numeric(df_ages_y[age_col_n], errors="coerce")
        df_ages_y[male_col]  = pd.to_numeric(df_ages_y[male_col],  errors="coerce")
        df_ages_y[female_col]= pd.to_numeric(df_ages_y[female_col],errors="coerce")
        df_ages_y = df_ages_y.dropna(subset=[age_col_n, male_col, female_col])

        # Bin into 5-year groups
        bins   = list(range(0, 86, 5)) + [120]
        labels = [f"{b}–{b+4}" for b in range(0, 85, 5)] + ["85+"]
        df_ages_y["age_grp"] = pd.cut(df_ages_y[age_col_n], bins=bins,
                                       labels=labels, right=False)
        pyramid = df_ages_y.groupby("age_grp", observed=True).agg(
            Male=(male_col, "sum"), Female=(female_col, "sum")
        ).reset_index()

        fig_pyr = go.Figure()
        fig_pyr.add_trace(go.Bar(
            y=pyramid["age_grp"].astype(str),
            x=-pyramid["Male"],
            orientation="h",
            name="Male",
            marker_color=POP_BLUE,
            hovertemplate="%{y}: %{customdata:,}<extra>Male</extra>",
        ...
        fig_pyr.update_layout(
            ...
            xaxis=dict(
                title="Population",
                tickvals=[-max_val * 0.75, -max_val * 0.5, -max_val * 0.25, 0,
                          max_val * 0.25, max_val * 0.5, max_val * 0.75],
                ticktext=[str(int(abs(v))) for v in
                          [-max_val * 0.75, -max_val * 0.5, -max_val * 0.25, 0,
                           max_val * 0.25, max_val * 0.5, max_val * 0.75]],
            ),
        ...
        # Insight: biggest cohort
        biggest = pyramid.loc[pyramid["Male"] + pyramid["Female"] == (pyramid["Male"] + pyramid["Female"]).max(), "age_grp"].iloc[0]
        st.markdown(insight_box(
            f"The largest age cohort is <b>{biggest}</b> years. "
            "Magdeburg has a relatively older demographic profile compared to Germany's national average — "
            "a legacy of emigration by younger residents post-reunification. "
            "Recent in-migration of students and international workers is gradually shifting the balance."
        ), unsafe_allow_html=True)
        st.caption(f"Source: KISS-MD / Einwohnermeldeamt · {age_yr}")
    else:
        st.info("Age/gender column names not found in dataset.")
except Exception as e:
    st.info(f"Age structure data unavailable: {e}")

# ─────────────────────────────────────────────────────────────────────────────
# SECTION 4: BIRTHS, DEATHS & MIGRATION
# ─────────────────────────────────────────────────────────────────────────────
